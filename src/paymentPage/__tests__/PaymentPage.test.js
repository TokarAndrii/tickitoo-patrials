...
describe("PaymentPage tests", ()=>{
    ...
    it(`should render Passenger qty - ${props.paymentPageInfo.passengersList.length}` , () => {
        expect(wrapper.find(Passenger).length).toEqual(props.paymentPageInfo.passengersList.length);
    });
    ...
    it(`should render <div class="payment-data__info-description"> with  numberTrip=${props.paymentPageInfo.tripDetailsInfo.numberTrip}}`, () => {
        expect(wrapper.find(".payment-data__info-description").contains(props.paymentPageInfo.tripDetailsInfo.numberTrip)).toEqual(true);
    });

    
    it(`should render <div class="payment-data__info-description"> with  numberTrip=${props.paymentPageInfo.tripDetailsInfo.carrier}}`, () => {
        expect(wrapper.find(".payment-data__info-description").contains(props.paymentPageInfo.tripDetailsInfo.carrier)).toEqual(true);
    });

    it(`should render <div class="payment-data__info-description"> with  numberTrip=${props.paymentPageInfo.tripDetailsInfo.bus}}`, () => {
        expect(wrapper.find(".payment-data__info-description").contains(props.paymentPageInfo.tripDetailsInfo.bus)).toEqual(true);
    });

    it(`should render  error message after handleToggleError method called`, () => {
        wrapper.instance().handleToggleError();
        expect(wrapper.find(".warning-message").length).toBe(1);
    });
